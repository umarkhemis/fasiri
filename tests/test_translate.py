...
from app.core.security import create_key, get_dev_key, hash_api_key, register_permanent_key
...
@pytest.fixture(scope="module")
def admin_key():
    """A permanent admin key used in tests that create new keys."""
    import os
    from app.core.security import PREFIX
    key = PREFIX + os.urandom(20).hex()
    register_permanent_key(key, "admin")
    return key
...
class TestAuthentication:
    ...
    def test_issue_new_key(self, client, admin_headers):
        resp = client.post("/api/v1/auth/keys", json={"name": "test-key"}, headers=admin_headers)
    ...
    def test_issued_key_is_usable(self, client, admin_headers):
        # Issue a key using admin credentials, then use it immediately
        resp = client.post("/api/v1/auth/keys", json={"name": "functional-test"}, headers=admin_headers)
